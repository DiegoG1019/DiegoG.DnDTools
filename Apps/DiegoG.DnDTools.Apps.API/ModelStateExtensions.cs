...
public static class FormCollectionExtensions
{
    ...
    )
    {
        ...
        if (parameters.Length > 0)
        {
            for (int i = 0; i < parameters.Length; i++)
            {
                ...
                {
                    (errors ??= []).Add($"NoEntry.{p.Name}");
                    continue;
                }
                else if (TryConvertValue(p.ParameterType, value[0]!, p.Name, out converted, out var exception) is false)
                {
                    if (exception is null)
                        (errors ??= []).Add($"CannotConvert.{p.Name}");
                    else
                        (errors ??= []).Add(new ModelError(exception, $"CannotConvert.{p.Name}"));
                    continue;
                }
            ...
            out Exception? error)
    {
        ...
        if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Nullable<>))
            return !string.IsNullOrEmpty(value) && TryConvertValue(Nullable.GetUnderlyingType(type)!, value, property, out result, out error);
